/*
	YTImg Client-side Thumbnail modifier
	Served as www.gstatic.com
	Ben Mullan (c) 2023
*/
...
/*
	CHOOSE THE EFFECT TO APPLY HERE! Choose from:
		- ShiftAllThumbnailsByOne
		- RotateAllThumbnails
		- SkewAndSaturateAllThumbnails
*/
setInterval(window.SkewAndSaturateAllThumbnails, 400);
...
window.ThumbnailsQuerySelectorString = "#thumbnail > yt-image > img";
//window.ThumbnailsQuerySelectorString = `#contents img.style-scope.yt-img-shadow[width="9999"]`;
//window.ThumbnailsQuerySelectorString = "img.style-scope.yt-img-shadow";

console.warn("%c\r\n\r\n-----------------------------------\r\n\r\nThumbnail Script Injected\r\n\r\n-----------------------------------\r\n\r\n", "background-color: #134023; color: #ABABAB; font-size: 200%;");
...
window.ShiftAllThumbnailsByOne = 
function () {
	console.log("%cRunning Thumbnail shifting...", "background-color: #134023; color: #ABABAB; font-size: 160%;");
	var _ThumbnailElements = document.querySelectorAll(window.ThumbnailsQuerySelectorString);
	for (var _ThumbnailElementIndex = 0; _ThumbnailElementIndex != (document.querySelectorAll("#thumbnail > yt-image > img").length - 1); ++_ThumbnailElementIndex) {
		/* If the Thumbnail has been loaded in yet, AND it hasn't already been modified... */
		if (
				_ThumbnailElements[_ThumbnailElementIndex].getAttribute("src")
				&& (!_ThumbnailElements[_ThumbnailElementIndex].getAttribute("data-already-shifted"))
			) {
			console.log("%cShifting Thumbnail (" + _ThumbnailElements[_ThumbnailElementIndex].getAttribute("src").split("/")[4] + ")", "background-color: #134023; color: #ABABAB; font-size: 120%;");
			
			/* Now perform the modification */
			_ThumbnailElements[_ThumbnailElementIndex].setAttribute(
				"src",
				_ThumbnailElements[_ThumbnailElementIndex + 1].getAttribute("src")
			);
			
			_ThumbnailElements[_ThumbnailElementIndex].setAttribute("data-already-shifted", "true");
		}
	}
}

window.RotateAllThumbnails = 
function () {
	console.log("%cRunning Thumbnail rotation...", "background-color: #134023; color: #ABABAB; font-size: 160%;");
	var _ThumbnailElements = document.querySelectorAll(window.ThumbnailsQuerySelectorString);
	for (var _ThumbnailElementIndex = 0; _ThumbnailElementIndex != (document.querySelectorAll("#thumbnail > yt-image > img").length - 1); ++_ThumbnailElementIndex) {
		/* If the Thumbnail has been loaded in yet, AND it hasn't already been modified... */
		if (
				_ThumbnailElements[_ThumbnailElementIndex].getAttribute("src")
				&& (!_ThumbnailElements[_ThumbnailElementIndex].getAttribute("data-already-rotated"))
			) {
			console.log("%cRotating Thumbnail (" + _ThumbnailElements[_ThumbnailElementIndex].getAttribute("src").split("/")[4] + ")", "background-color: #134023; color: #ABABAB; font-size: 120%;");
			
			/* Now perform the modification */
			_ThumbnailElements[_ThumbnailElementIndex].style.transform = ("rotate(" + GetRandomIntBetween(-50, 50).toString() + "deg)");
						
			_ThumbnailElements[_ThumbnailElementIndex].setAttribute("data-already-rotated", "true");
		}
	}
}

window.SkewAndSaturateAllThumbnails = 
function () {
	console.log("%cRunning Thumbnail Skewing and Saturating...", "background-color: #134023; color: #ABABAB; font-size: 160%;");
	var _ThumbnailElements = document.querySelectorAll(window.ThumbnailsQuerySelectorString);
	for (var _ThumbnailElementIndex = 0; _ThumbnailElementIndex != (document.querySelectorAll("#thumbnail > yt-image > img").length - 1); ++_ThumbnailElementIndex) {
		/* If the Thumbnail has been loaded in yet, AND it hasn't already been modified... */
		if (
				_ThumbnailElements[_ThumbnailElementIndex].getAttribute("src")
				&& (!_ThumbnailElements[_ThumbnailElementIndex].getAttribute("data-already-skewed"))
			) {
			console.log("%cSkewing and Saturating Thumbnail (" + _ThumbnailElements[_ThumbnailElementIndex].getAttribute("src").split("/")[4] + ")", "background-color: #134023; color: #ABABAB; font-size: 120%;");
			
			/* Now perform the modification */
			_ThumbnailElements[_ThumbnailElementIndex].style.transform = "rotate(" + GetRandomIntBetween(-100, 100).toString() + "deg) skewY(18deg) translate(-10px,10px) scale(1.6)";
			_ThumbnailElements[_ThumbnailElementIndex].style.filter = "saturate(" + GetRandomIntBetween(1, 4).toString() + ") invert(" + (GetRandomIntBetween(0, 50)/100).toString() + "%) blur(" + GetRandomIntBetween(0, 1).toString() + "px)";
						
			_ThumbnailElements[_ThumbnailElementIndex].setAttribute("data-already-skewed", "true");
		}
	}
}
